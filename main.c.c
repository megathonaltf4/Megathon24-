...
// Check for item collection - add this after player movement
 ...
 if ((projectiles[i].direction == 1 &&
 ((projectiles[i].y == -400 && projectiles[i].x <= 0) ||
 (projectiles[i].y == -315 && projectiles[i].x <= 1400))) ||
 (projectiles[i].direction == 2 &&
 (projectiles[i].y == -250 && projectiles[i].x >= 1800))) {
 projectiles[i].active = false;
 continue;
 }

 for (int j = 0; j < 37; j++) {
 if (checkCollision(projectiles[i].x, projectiles[i].y,
 projectiles[i].radius, lines[j])) {
 projectiles[i].active = false;
 break;
 }
 }
 ...
 for (int i = 0; i < 37; i++) {
 if (checkCollision(ballX, ballY, ballRadius, lines[i])) {
 ballX = prevBallX;
 ballY = prevBallY;
 break;
 }
 ...
 for (int i = 0; i < MAX_SPIKES; i++) {
 Line spikeLine = {spikes[i].baseX1, spikes[i].baseY,
 spikes[i].currentX, spikes[i].baseY};
 if (checkCollision(ballX, ballY, ballRadius, spikeLine)) {
 ballX = prevBallX;
 ballY = prevBallY;
 break;
 }
